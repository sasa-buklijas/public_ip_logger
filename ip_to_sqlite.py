...
def generate_webpage():
    ...
    # Footer
    html.write(f"<p>Generated by ip_logger version {get_version()}</p></body></html>\n")
...
def main():
    uld = Path(platformdirs.user_log_dir(PACKAGE_NAME, ensure_exists=True))
    #print(f'{uld=}')
    ...
    parser = argparse.ArgumentParser(description=f"Public IP to SQLite v{version}")
    # Optional flag-like positional argument
    parser.add_argument(
        'mode',
        nargs='?',            # optional
        default='run',        # default is 'run'
    ...
    if args.mode == 'dirs':
        udd = Path(platformdirs.user_data_dir(PACKAGE_NAME, ensure_exists=True))
        uld = Path(platformdirs.user_log_dir(PACKAGE_NAME, ensure_exists=True))
        print(f'Program data directory: {udd}')
        print(f'Program  log directory: {uld}')
        exit(0)
